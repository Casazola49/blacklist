<template>
  <div class="min-h-screen bg-gradient-to-br from-gray-900 via-black to-gray-900 flex items-center justify-center p-4">
    <div class="max-w-md w-full">
      <HologramCard class="p-8">
        <div class="text-center mb-8">
          <div class="text-3xl font-bold text-brand-primary mb-2">
            <GlitchText text="CONSERJE" />
          </div>
          <p class="text-gray-400">Panel de Administración</p>
          <p class="text-sm text-gray-500 mt-2">Acceso restringido solo para administradores</p>
        </div>

        <div v-if="authStore.error" class="mb-6 p-4 bg-red-900/20 border border-red-500/30 rounded-lg">
          <p class="text-red-300 text-sm">{{ authStore.error }}</p>
        </div>

        <div class="space-y-4">
          <NeonButton 
            @click="handleAdminLogin"
            :disabled="authStore.loading"
            variant="primary"
            class="w-full"
          >
            <div v-if="authStore.loading" class="flex items-center justify-center">
              <CyberLoader size="small" />
              <span class="ml-2">Verificando acceso...</span>
            </div>
            <div v-else class="flex items-center justify-center">
              <svg class="w-5 h-5 mr-2" viewBox="0 0 24 24" fill="currentColor">
                <path d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z"/>
                <path d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z"/>
                <path d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l2.85-2.22.81-.62z"/>
                <path d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z"/>
              </svg>
              Acceder con Google
            </div>
          </NeonButton>

          <div class="text-center">
            <button 
              @click="$router.push('/')"
              class="text-gray-400 hover:text-white text-sm transition-colors"
            >
              ← Volver al inicio
            </button>
          </div>
        </div>

        <div class="mt-8 pt-6 border-t border-gray-700">
          <div class="text-xs text-gray-500 text-center">
            <p class="mb-2">⚠️ Solo para personal autorizado</p>
            <p>Este panel requiere credenciales de administrador válidas.</p>
          </div>
        </div>
      </HologramCard>
    </div>
  </div>
</template>

<script setup lang="ts">
import { useRouter } from 'vue-router'
import { useAuthStore } from '../stores/auth'
import HologramCard from '../components/ui/HologramCard.vue'
import GlitchText from '../components/ui/GlitchText.vue'
import NeonButton from '../components/ui/NeonButton.vue'
import CyberLoader from '../components/ui/CyberLoader.vue'

const router = useRouter()
const authStore = useAuthStore()

const handleAdminLogin = async () => {
  try {
    authStore.clearError()
    const success = await authStore.adminLogin()
    
    if (success) {
      router.push('/dashboard/admin')
    }
  } catch (error) {
    console.error('Admin login error:', error)
  }
}
</script>