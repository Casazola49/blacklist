...
exports.getCleanupStatistics = exports.triggerManualCleanup = exports.cleanOldAuditLogs = exports.cleanInactiveSessions = exports.cleanTempFiles = exports.cleanOldNotifications = exports.cleanExpiredQRCodes = void 0;
...
exports.cleanExpiredQRCodes = (0, scheduler_1.onSchedule)('0 2 * * *', async () => {
    const startTime = Date.now();
    let deletedCount = 0;
    const errors = [];
    try {
        firebase_functions_1.logger.info('Starting cleanup of expired QR codes');
        ...
        if (expiredQRQuery.empty) {
            firebase_functions_1.logger.info('No expired QR codes found');
            return;
        }
        ...
        expiredQRQuery.docs.forEach(doc => {
            batch.update(doc.ref, {
                estado: 'expirado',
                fechaExpiracion: firestore_1.FieldValue.serverTimestamp()
            });
            deletedCount++;
        });
        await batch.commit();
        ...
        firebase_functions_1.logger.info(`Cleaned up ${deletedCount} expired QR codes in ${executionTime}ms`);
        // Log cleanup result
        ...
        firebase_functions_1.logger.error('Error cleaning expired QR codes:', error);
...
exports.cleanOldNotifications = (0, scheduler_1.onSchedule)('0 3 * * 0', async () => {
    const startTime = Date.now();
    let deletedCount = 0;
    const errors = [];
    try {
        firebase_functions_1.logger.info('Starting cleanup of old notifications');
        ...
        for (const userDoc of usersQuery.docs) {
            try {
                ...
                const recentNotifications = notifications.filter((notif) => {
                    var _a;
                    const notifDate = ((_a = notif.fechaCreacion) === null || _a === void 0 ? void 0 : _a.toDate()) || new Date(0);
                ...
                firebase_functions_1.logger.error(`Error cleaning notifications for user ${userDoc.id}:`, error);
                errors.push(`User ${userDoc.id}: ${error instanceof Error ? error.message : 'Unknown error'}`);
            }
        }
        const executionTime = Date.now() - startTime;
        firebase_functions_1.logger.info(`Cleaned up ${deletedCount} old notifications in ${executionTime}ms`);
        ...
        firebase_functions_1.logger.error('Error cleaning old notifications:', error);
        errors.push(error instanceof Error ? error.message : 'Unknown error');
        await logCleanupResult('old_notifications', {
            elementosEliminados: deletedCount,
            tiempoEjecucion: Date.now() - startTime,
...
exports.cleanTempFiles = (0, scheduler_1.onSchedule)('0 4 * * *', async () => {
    const startTime = Date.now();
    let deletedCount = 0;
    const errors = [];
    try {
        firebase_functions_1.logger.info('Starting cleanup of temporary files');
        ...
        for (const file of files) {
            try {
                ...
                firebase_functions_1.logger.error(`Error deleting temp file ${file.name}:`, error);
        ...
        const executionTime = Date.now() - startTime;
        firebase_functions_1.logger.info(`Cleaned up ${deletedCount} temporary files in ${executionTime}ms`);
        ...
        firebase_functions_1.logger.error('Error cleaning temporary files:', error);
        errors.push(error instanceof Error ? error.message : 'Unknown error');
        await logCleanupResult('temp_files', {
            elementosEliminados: deletedCount,
            tiempoEjecucion: Date.now() - startTime,
...
exports.cleanInactiveSessions = (0, scheduler_1.onSchedule)('0 1 * * *', async () => {
    const startTime = Date.now();
    let deletedCount = 0;
    const errors = [];
    try {
        firebase_functions_1.logger.info('Starting cleanup of inactive sessions');
        ...
        for (const userDoc of usersQuery.docs) {
            try {
                ...
                firebase_functions_1.logger.error(`Error cleaning FCM tokens for user ${userDoc.id}:`, error);
                errors.push(`User ${userDoc.id}: ${error instanceof Error ? error.message : 'Unknown error'}`);
            }
        }
        const executionTime = Date.now() - startTime;
        firebase_functions_1.logger.info(`Cleaned up ${deletedCount} inactive sessions in ${executionTime}ms`);
        ...
        firebase_functions_1.logger.error('Error cleaning inactive sessions:', error);
        errors.push(error instanceof Error ? error.message : 'Unknown error');
        await logCleanupResult('inactive_sessions', {
            elementosEliminados: deletedCount,
            tiempoEjecucion: Date.now() - startTime,
...
exports.cleanOldAuditLogs = (0, scheduler_1.onSchedule)('0 5 1 * *', async () => {
    const startTime = Date.now();
    let deletedCount = 0;
    const errors = [];
    try {
        firebase_functions_1.logger.info('Starting cleanup of old audit logs');
        ...
        if (!importantLogsQuery.empty) {
            ...
            importantLogsQuery.docs.forEach(doc => {
                ...
                archiveBatch.set(archiveRef, Object.assign(Object.assign({}, doc.data()), { archivedAt: firestore_1.FieldValue.serverTimestamp() }));
            });
            await archiveBatch.commit();
        }
        const executionTime = Date.now() - startTime;
        firebase_functions_1.logger.info(`Cleaned up ${deletedCount} old audit logs in ${executionTime}ms`);
        ...
        firebase_functions_1.logger.error('Error cleaning old audit logs:', error);
        errors.push(error instanceof Error ? error.message : 'Unknown error');
        await logCleanupResult('old_logs', {
            elementosEliminados: deletedCount,
            tiempoEjecucion: Date.now() - startTime,
...
exports.triggerManualCleanup = (0, https_1.onCall)(async (request) => {
    ...
    try {
        ...
        if (!admin || admin.tipo !== 'admin') {
            throw new https_1.HttpsError('permission-denied', 'Not authorized to trigger cleanup');
        }
        firebase_functions_1.logger.info(`Manual cleanup triggered by admin ${adminId} for task ${taskType}`);
        let result;
        switch (taskType) {
            case 'expired_qr':
            ...
            case 'all':
                ...
                break;
            default:
                throw new https_1.HttpsError('invalid-argument', 'Invalid task type');
        }
        // Log admin action
        ...
        firebase_functions_1.logger.error('Error in manual cleanup:', error);
        throw new https_1.HttpsError('internal', 'Failed to execute cleanup');
    }
});
// Get cleanup statistics
exports.getCleanupStatistics = (0, https_1.onCall)(async (request) => {
    ...
    try {
        ...
        const admin = adminDoc.data();
        if (!admin || admin.tipo !== 'admin') {
            throw new https_1.HttpsError('permission-denied', 'Not authorized to view cleanup statistics');
        }
        ...
        firebase_functions_1.logger.error('Error getting cleanup statistics:', error);
        throw new https_1.HttpsError('internal', 'Failed to get cleanup statistics');
    }
});
// Helper function to execute cleanup tasks
async function executeCleanupTask(taskType, dryRun = false) {
    const startTime = Date.now();
...
async function logCleanupResult(taskId, result) {
    try {
        await db.collection('cleanup_results').add(Object.assign({ taskId, fecha: firestore_1.FieldValue.serverTimestamp() }, result));
    }
    catch (error) {
        firebase_functions_1.logger.error('Error logging cleanup result:', error);
    }
}
